...
const StudentInvite = () => {
  ...
  useEffect(() => {
    ...
    const verifyInvite = async () => {
      ...
      try {
        const invite = await superAdminApi.verifyStudentInvite(token, email);
        if (!active) return;
        setName(invite.name);
      } catch (err: unknown) {
        if (!active) return;
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const result = await superAdminApi.acceptStudentInvite(
        token,
        email,
        password,
      );
      ...
      <div className="mx-auto flex min-h-[calc(100vh-5rem)] w-full max-w-md items-center">
        <Card className="w-full">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              Complete your invitation to start learning on TernKonnect Academy.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {checking ? (
